/**
 * GSC Pilot v01 — Marge et prix de vente
 ...
 * vente. Confirmée à l'identique dans backup.js (le même niveau régit les
 * deux). Source : table active du prototype v19 (aucune version divergente
 * trouvée pour celle-ci — une seule définition dans tout le fichier).
 */
export const COMPLEXITY_MARKUP_SCALE = Object.freeze({
  0: 20, 1: 22, 2: 23, 3: 24, 4: 25, 5: 30,
  6: 35, 7: 38, 8: 42, 9: 50, 10: 60,
});

export function complexityMarkup(level) {
  const clamped = Math.max(0, Math.min(10, Math.round(Number(level || 0))));
...
export function saleFromCost(baseCost, marginPct) {
 ...
 */
export function projectMargin(sold, laborCost, purchases) {
