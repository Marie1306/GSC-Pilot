/**
 * GSC Pilot v01 — Sous-assemblages de conception
 *
...
export function declareSubassemblyReady(subassemblies, { projectId, number, declaredBy }) {
 ...
 * s'appliquent.
 */
export function markPartsListReady(subassembly, preparedBy, hoursByCategory) {
  if (subassembly.status !== "pending_parts_list") {
    throw new Error(`Sous-assemblage ${subassembly.id} n'est pas en attente de liste de pièces (statut actuel : ${subassembly.status}).`);
  }
 ...
 */
export function declareAssemblyReady(subassembly, declaredBy) {
 ...
 */
export function subassemblyGanttTasks(subassembly) {
  if (subassembly.status !== "ready_for_production") {
    throw new Error(`Sous-assemblage ${subassembly.id} n'est pas prêt pour la production.`);
  }
  const isFabrication = (category) => category === "fabrication" || category.startsWith("fabrication-");
  const fabricationIds = Object.keys(subassembly.hoursByCategory)
    .filter(isFabrication)
    .map((category) => `${subassembly.id}-${category}`);

  const tasks = [];
  Object.entries(subassembly.hoursByCategory).forEach(([category, hours]) => {
...
export function designerHistory(subassemblies, declaredBy) {
