/**
 * GSC Pilot v01 — Avenants (changements de commande)
 *
...
function isBackupEligible(category) {
 ...
 */
export function calculateAmendment(hoursByCategory, { marginPct, backupPct, projectBackupRate, purchases = 0, internalRates = AMENDMENT_INTERNAL_RATES }) {
  let laborHours = 0;
  let laborCost = 0;
 ...
 * que `billing.js` — confirmé, comportement déjà correct dans la v19).
 */
export function applyAmendmentToProject(project, calc) {
 ...
 */
export function amendmentTasks(amendmentId, hoursByCategory, internalRates = AMENDMENT_INTERNAL_RATES) {
 ...
 * mécanisme que les extras d'installation (voir billing.js) plutôt qu'un
 ...
 * facturation.
 */
export function amendmentInvoiceRequest(amendmentId, calc) {
  return {
