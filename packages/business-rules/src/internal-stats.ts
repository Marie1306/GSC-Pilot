/**
 * GSC Pilot — Statistiques internes (« Amélioration GSC »)
 *
 ...
 *
 * Porté depuis docs/handoff/03-modules-v01/internal-stats.js — typage
 * ajouté uniquement, aucune ligne de logique changée (voir CLAUDE.md).
 */
...
function yearOf(dateString: string | undefined): number | null {
  const year = Number(String(dateString || "").slice(0, 4));
  return Number.isFinite(year) ? year : null;
}

export interface InternalTimeEntry {
  employee: string;
  projectType: string;
  status: string;
  date?: string;
...
export interface InternalHoursByEmployee {
  employeeId: string;
  hours: number;
  value: number;
  count: number;
...
export interface InternalHoursSummary {
  employees: InternalHoursByEmployee[];
  hours: number;
  value: number;
}
 ...
 */
export function internalHoursSummary(timeEntries: InternalTimeEntry[] | undefined, year: number): InternalHoursSummary {
  const entries = (timeEntries || []).filter(
    (entry) => entry.projectType === "internal" && entry.status === "approved" && yearOf(entry.date || entry.approvedAt) === year,
  );
  const byEmployee = new Map<string, InternalHoursByEmployee>();
...
export interface InternalPurchasesByCategory {
  category: string;
  amount: number;
  count: number;
}

export interface InternalPurchasesSummary {
  categories: InternalPurchasesByCategory[];
  amount: number;
  count: number;
}
 ...
 * ensemble).
 */
export function internalPurchasesSummary(purchases: InternalPurchase[] | undefined, year: number): InternalPurchasesSummary {
  ...
  const byCategory = new Map<string, InternalPurchasesByCategory>();
